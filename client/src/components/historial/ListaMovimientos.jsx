...
function FilaMovimiento({ venta }) {
  return (
    // Fase 6: aca va a colgar la edicion/anulacion (solo Admin). Por ahora
    // la fila ya es un boton real, tocable, para no tener que rehacer el
    // diseño cuando se conecte esa accion.
    <button
      type="button"
      className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-crema/70"
    >
      <span
      ...
      <div className="flex shrink-0 items-center gap-1">
        ...
        <IconChevronRight width={16} height={16} className="text-texto-secundario/60" />
      </div>
    </button>
  );
}

export default function ListaMovimientos({ ventas }) {
  if (!ventas.length) {
        ...
        <div key={grupo.etiqueta}>
          ...
          <div className="divide-y divide-borde-tarjeta rounded-[20px] border border-borde-tarjeta bg-white shadow-sm">
            {grupo.ventas.map((venta) => (
              <FilaMovimiento key={venta.id} venta={venta} />
            ))}
          </div>
        </div>
      ))}
